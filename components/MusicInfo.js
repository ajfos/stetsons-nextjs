...
export default function MusicInfo({ numberToShow = 0, showTitle }) {
  ...
  const moreImage =
    releaseData.releases[Math.floor(Math.random() * Math.floor(releaseData.releases.length - numberToShow))].artwork;
  return (
    <>
      {showTitle && <div className={styles.header}>Music</div>}
      <div className={styles.musicInfo}>
        {releases.map((release, index) => (
          <Link legacyBehavior href={`/releases/${release.id}`} key={release.id}>
